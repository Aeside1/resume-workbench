...
几个关键边界（完整口径见 [docs/CONTEXT.md](docs/CONTEXT.md)）：
...
**经历资产管理是这个项目的核心，已经完善**：身份、经历分组、具体工作内容、简历亮点与沉浸编辑构成一条自洽的闭环，是目前对外提供的主要能力。

**简历组合（简历方案）仍在开发中**：它已经能跑通「列表 → 编排 → 文稿 → 留档」的链路，但**整体结构与边界尚未定稿**，后续版本很可能对它做**破坏性重构**——数据模型、接口路径、概念命名都可能改。现阶段请当成预览，不要作为稳定依赖；对外承诺的能力以经历资产管理为准。
...
| 经历分组 | 创建、编辑、排序、归档、恢复、彻底删除 |
| 具体工作内容 | 创建、编辑、排序、归档、恢复、彻底删除；详细记录 / 技术材料 / 结果数据 / 补充说明 |
| 简历亮点 | 同一工作内容下多条亮点：创建、编辑、复制、排序、归档、恢复、彻底删除（被引用时拒绝并提示） |
| 沉浸编辑 | 富文本 Markdown 编辑（Milkdown / ProseMirror）、自动保存与保存状态提示、专注写作区 |

### 开发中：简历组合（结构未定）

以下能力已经可用，但均属于**可能发生破坏性重构**的范围：

| 模块 | 现在能做什么 |
| --- | --- |
| 简历方案 | 创建 / 改名 / 用途 / 归档 / 恢复 / 彻底删除 / 复制 |
...
docs/
  PRD.md                                    产品需求
  ARCHITECTURE.md                           系统架构与前后端分工
  CONTEXT.md                                领域词汇（术语唯一来源）
  adr/                                      架构决策记录
  agents/                                   协作约定（issue tracker / triage 标签 / 领域文档结构）
AGENTS.md                                   本仓库的协作约定
```
...
| 文档 | 回答的问题 |
| --- | --- |
| [docs/PRD.md](docs/PRD.md) | 用户为什么需要这个产品，第一版要解决什么 |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | 系统如何承载产品能力，前后端如何分工 |
| [docs/CONTEXT.md](docs/CONTEXT.md) | 每个领域词汇具体指什么（术语唯一来源） |
| [docs/adr/](docs/adr/) | 已确认的架构决策及其理由 |
| [docs/agents/](docs/agents/) | 协作约定：issue tracker、triage 标签、领域文档结构 |
| [AGENTS.md](AGENTS.md) | 本仓库的协作约定（供 AI 协作与人类贡献者参考） |

需求规格与开发任务（issue tracker）维护在仓库外的本地目录，不随代码发布。
...
7. **自动保存在慢网下可能丢尾**：保存请求在途时的后续输入存在不落库的可能（已用慢网限速复现并登记，尚未修复）。

## 路线图

- **v0.1（当前）**：经历资产管理完善（核心能力）；简历组合可用但结构未定。
- **下一步**：简历组合定稿，补上导出快照与方案内导出历史，补齐 PRD 第一版核心闭环的最后一段。
...
- 需求规格与开发任务用本地 Markdown 维护（**不入版本库**），约定见 [docs/agents/issue-tracker.md](docs/agents/issue-tracker.md)。
